...
export const TitleSearch: React.FC<SelectProps> = ({
    genreList,
    id,
    name
})  => {
    ...
    useEffect(() => {
        if (name != null && name != "") {
            const fetchMovie = () => {
                fetch('http://localhost:8080/api/movies/title?api_key=' + api + "&query=" + name, requestOptions)
            ...
            fetchMovie();
            setSearchTitle(name != null ? name.replace("%20", " ") : "");
            //setLoading(false);
        }
        else {
